  ...
  return (
    <ListGroup className="rounded-0 position-fixed bottom-0 top-0 d-none d-md-block bg-black z-2" style={{ width: 120 }} id="wd-kambaz-navigation">
      ...
      <ListGroupItem as={Link} href="/Account"
        className={`bg-black border-0 text-center ${pathname.includes("Account") ? "text-danger bg-white" : "text-white bg-black"}`}>
        <FaRegUserCircle className={`fs-1 ${pathname.includes("Account") ? "bg-white text-danger" : "bg-black text-white"}`} />
        <br />
        Account
      </ListGroupItem>
      {links.map((link) => (
        <ListGroupItem key={link.path} as={Link} href={link.path}
          className={`bg-black border-0 text-center ${pathname.includes(link.label) ? "text-danger bg-white" : "bg-black text-white"}`}>
